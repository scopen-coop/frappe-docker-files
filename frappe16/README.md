...
```sh
 mkdir -p  /opt/data/frappe16/mysql
```
Assuming now /opt/data/frappe16/mysql directory exists 
```sh
 docker volume create --driver local --opt type=none --opt device=/opt/data/frappe16/mysql --opt o=bind frappe16-mariadb-vol
```

Create directory if not exists
```sh
 mkdir -p  /opt/data/frappe16/redis_cache;
 mkdir -p  /opt/data/frappe16/redis_queue;
 mkdir -p  /opt/data/frappe16/redis_socketio; 
```

Assuming now /opt/data/frappe16/redis_cache/, /opt/data/frappe16/redis_queue/,  /opt/data/frappe16/redis_socketio directories exists
```sh
docker volume create --driver local --opt type=none --opt device=/opt/data/frappe16/redis_cache --opt o=bind frappe16-redis-cache-data;
docker volume create --driver local --opt type=none --opt device=/opt/data/frappe16/redis_queue --opt o=bind frappe16-redis-queue-data;
docker volume create --driver local --opt type=none --opt device=/opt/data/frappe16/redis_socketio --opt o=bind frappe16-redis-socketio-data;
```
...
/home/..../frappe_docker_16 => git clone https://github.com/frappe/frappe_docker frappe_docker_16
...
```sh
    cd /home/..../frappe_docker_file/frappe16
...
```sh
docker exec -e "TERM=xterm-256color" -w /workspace/development -it frappe16-frappe-1 bash
```
...
bench --site erpnext16.local clear-cache;
